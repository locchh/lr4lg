...
programIdParagraph
    : PROGRAM_ID (DOT | DOT_FS) WS? programName (
...
authorParagraph
    : AUTHOR (DOT | DOT_FS) WS? (author_name DOT_FS)? commentEntry?
    ;
...
dateWrittenParagraph
    : DATE_WRITTEN (DOT | DOT_FS) (IDENTIFIER | literal | WS)* DOT_FS? commentEntry?
    ;

// - date compiled paragraph ----------------------------------

dateCompiledParagraph
    : DATE_COMPILED (DOT | DOT_FS) (IDENTIFIER | literal | WS)* DOT_FS? commentEntry?
    ;
...
sourceComputerParagraph
    : SOURCE_COMPUTER (DOT | DOT_FS) WS? computerName (WITH? DEBUGGING MODE)? DOT_FS
    ;

// - object computer paragraph ----------------------------------

objectComputerParagraph
    : OBJECT_COMPUTER (DOT | DOT_FS) WS? computerName objectComputerClause* DOT_FS
    ;
...
dataDescriptionEntry
    ...
    | copyStatement (DOT | DOT_FS)
    ;

// copy statement

copyStatement
    : COPY copySource ((directoryPhrase | familyPhrase | replacingPhrase | SUPPRESS))*
    ;
...
addTo
    : (identifier | figurativeConstant) ROUNDED?
    ;
...
NUMERICLITERAL2
    : (PLUSCHAR | MINUSCHAR)? Z* (DOT | COMMACHAR) [0-9Z]+ (
        ('e' | 'E') (PLUSCHAR | MINUSCHAR)? [0-9]+
    )?
    ;
